...
def run_model(lambda_selected, h, window_size, x, target, kernel_choice='gaussian'):
    ...
    bins_structure_tune = CollectionOfBins(range_of_bin=2)
    bins_tune = bins_structure_tune.organiseLabelsIntoBins(target)
    ...
    for i in range(len(x)):
        ...
        if count_window == window_size or count_window >= len(x) - i:
            ...
            for label in train_y:
                ...
                bin_of_last_label = bins_structure_tune.findBinOflabel(label, bins_tune)
                for bin in bins_tune:
                    bin.weight = weight_update(instances_seen, bin.weight, bin.lower_bound,
                                               bin_of_last_label.lower_bound, h, kernel_choice, sigma)

                instances_seen += 1  # Increment counter

            # Nomrlaize weights
            normalized_incremental_bins = bins_tune.copy()
            ...
            for instance_idx in range(len(train_x)):
                ...
                # PRedict
                lds_pred = standard_tree_lds.predict(train_x[instance_idx])[0]
                ht_lds_predictions.append(lds_pred)

                hs_lds_pred = standard_tree_lds.predictHSNew(train_x[instance_idx], lambda_selected=lambda_selected)[0]
                ht_lds_hs_predictions.append(hs_lds_pred)

                # Fit
                bin_of_label = bins_structure_tune.findBinOflabel(train_y[instance_idx],
    ...
    print(f"HT + LDS + HS with window size :{window_size}, MAE={ht_lds_hs_mae:.4f}, MSE={ht_lds_hs_mse:.4f}, R2={ht_lds_hs_r2:.4f}")
                ...
                for i in range(len(x)):
                    ...
                    if count_window == window_size or count_window >= len(x) - i:
                        ...
                        for instance_idx in range(len(train_x)):
                            ...
                            bin_of_label = bins_structure_tune.findBinOflabel(train_y[instance_idx],
                                                                         normalized_incremental_bins)
                            standard_tree_lds.partial_fit(X=train_x[instance_idx], y=[train_y[instance_idx]],
                                                          sample_weight=[bin_of_label.weight])
...
# TODO : maybe this part needs fixing on the LDS implementation (we'll see with the experiments)
def run_tune_model(x, target, kernel_choice, tune_metric_selection='mse'):
    ...
    bins_structure = CollectionOfBins(range_of_bin=2)
    bins = bins_structure.organiseLabelsIntoBins(target)
        ...
        if (counter_examples % 1000 == 0) and (counter_examples < 1000 or (counter_examples > 5000 and counter_examples <= 6000) or (counter_examples > 10000 and counter_examples <= 11000) or (counter_examples > 15000 and counter_examples <= 16000)):
            ...
            if count_window == best_parameters['best_window_size'] or count_window >= len(x) - i:
                ...
                for label in train_y:
                    ...
                    bin_of_last_label = bins_structure.findBinOflabel(label, bins)
                    for bin in bins:
                        bin.weight = weight_update(instances_seen, bin.weight, bin.lower_bound,
                                                   bin_of_last_label.lower_bound, best_parameters['best_h'], kernel_choice, sigma)

                    instances_seen += 1  # Increment counter


                # Nomrlaize weights
                normalized_incremental_bins1 = bins.copy()
                for bin in normalized_incremental_bins1:
                    bin.weight = 1/ (bin.weight + 1e-6)
                ...
                for instance_idx in range(len(train_x)):
                    ...
                    #Train
                    bin_of_label = bins_structure.findBinOflabel(train_y[instance_idx], normalized_incremental_bins1)
                    ht_lds.partial_fit(train_x[instance_idx], [train_y[instance_idx]], sample_weight=[bin_of_label.weight])
