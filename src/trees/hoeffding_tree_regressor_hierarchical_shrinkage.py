...
from ._nodes.htr_hs_nodes import compute_sd
from ._nodes.htr_hs_nodes import LearningNodePerceptronHS
...
def RegressionHoeffdingTree(max_byte_size=33554432, memory_estimate_period=1000000,
    ...
    return HoeffdingTreeRegressorHS(max_byte_size=max_byte_size,
...
class HoeffdingTreeRegressorHS(RegressorMixin, HoeffdingTreeClassifier):
    ...
    >>> ht_hs_reg = HoeffdingTreeRegressorHS()
    >>>
    ...
    >>>     y_pred[n_samples] = ht_hs_reg.predict(X)[0]
    >>>     ht_hs_reg.partial_fit(X, y)
    >>>     n_samples += 1
    >>>
    >>> # Display results
    >>> print('{} samples analyzed.'.format(n_samples))
    >>> print('Hoeffding Tree HS regressor mean absolute error: {}'.
    ...
    def _new_learning_node(self, initial_stats=None, parent_node=None, is_active=True):
        ...
        if is_active:
            if self.leaf_prediction == self._TARGET_MEAN:
                ...
                return LearningNodePerceptronHS(
                    initial_stats, parent_node, random_state=self.random_state, shrinkage_lambda=self.shrinkage_lambda
                )
        else:
            if self.leaf_prediction == self._TARGET_MEAN:
                return InactiveLearningNodeMean(initial_stats)
            elif self.leaf_prediction == self._PERCEPTRON:
                return InactiveLearningNodePerceptron(initial_stats, parent_node, random_state=self.random_state)
