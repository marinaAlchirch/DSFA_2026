...
from . import HoeffdingTreeRegressor
from src.utils import add_dict_values
...
def RegressionHAT(max_byte_size=33554432, memory_estimate_period=1000000, grace_period=200,
    ...
    return HoeffdingBalancedTreeRegressor(max_byte_size=max_byte_size,
...
class HoeffdingBalancedTreeRegressor(HoeffdingTreeRegressor):
    ...
    >>> from src.trees import HoeffdingBalancedTreeRegressor
    >>> import numpy as np
    >>>
    >>> # Setup a data stream
    ...
    >>> hat_reg = HoeffdingBalancedTreeRegressor()
    >>>
    ...
    def _partial_fit(self, X, y, weight):
        ...
        if self._tree_root and isinstance(self._tree_root, AdaActiveLearningNodeRegressor):
            dir_weights = self._tree_root._adwin_dir.compute_dir_weights()
            weight *= dir_weights.get(y, 1.0) # Apply LDS reweighting
