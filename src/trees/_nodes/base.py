...
from .._attribute_observer import AttributeObserverNull
...
class Node(metaclass=ABCMeta):
    ...
    def __init__(self, stats=None):
        self.stats = stats
# ################ Marina code starts here 11/15 ################
#         self.samples_seen = 0  # Marina added this
#
#
#     def samples_in_node(self, samples_seen):
#         self.samples_seen = samples_seen
#
#     def add_sample_in_node(self):
#             self.samples_seen += 1
# ################ Marina code ends here 11/15 ################
    ...
    def filter_instance_to_leaf(self, X, parent, parent_branch):
        ...
        X: numpy.ndarray of shape (n_samples, n_features)
           Data instances.
        parent: src.trees.nodes.Node or None
            Parent node.
        parent_branch: Int
            Parent branch index
    ...
    def filter_instance_to_leafHS(self, X, parent, parent_branch, path=None):
        """
        Traverse down the tree to locate the corresponding leaf for an instance
        and record the path.
        ...
        parent_branch: int
            Parent branch index.
        path: list or None
            List to store the path from root to the leaf.
        ...
        # Return the FoundNode and the path
        return FoundNode(self, parent, parent_branch), path
    ########## Marina code ends here 11/15/2024 ##############
...
class SplitNode(Node):
    ...
    def filter_instance_to_leafHS(self, X, parent, parent_branch, path=None):
        ...
        # Determine the child index for the instance
        child_index = self.instance_child_index(X)
        if child_index >= 0:
            child = self.get_child(child_index)
            if child is not None:
                # Recursively call on the child node, passing the updated path
                return child.filter_instance_to_leafHS(X, self, child_index, path)
            else:
                # Return the FoundNode and path when no child is found
                return FoundNode(None, self, child_index), path
        else:
            # Return the FoundNode and path for invalid branch indices
            return FoundNode(self, parent, parent_branch), path
