...
from src.drift_detection.adwin_dir import ADWIN_DIR
...
class AdaSplitNodeRegressor(AdaSplitNode):
    ...
    def learn_one(self, X, y, weight, tree, parent, parent_branch):
        ...
        # Compute DIR weights from ADWIN
        dir_weights = self._adwin.compute_dir_weights()
        weight *= dir_weights.get(y, 1.0)  # Apply reweighting
        if self._adwin_dir is None:
            self._adwin_dir = ADWIN_DIR()

        old_error = self.error_estimation

        # Add element to change detector
        self._adwin_dir.add_element(normalized_error)

        # Detect change
        self.error_change = self._adwin_dir.detected_change()
...
class AdaActiveLearningNodeRegressor(ActiveLearningNodePerceptron, AdaNode):
    ...
    def error_estimation(self):
        return self._adwin_dir.estimation

    @property
    def error_width(self):
        return self._adwin_dir.width

    def error_is_null(self):
        return self._adwin_dir is None
    ...
    def learn_one(self, X, y, weight, tree, parent, parent_branch):
        ...
        self._error_change = self._adwin_dir.detected_change()
