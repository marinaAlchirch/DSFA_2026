...
class HoeffdingTreeClassifier(BaseSKMObject, ClassifierMixin):
    ...
    def _get_votes_for_instance(self, X):
        ...
        if self._tree_root is not None:
            ...
            if not isinstance(leaf_node, SplitNode):
                ...
                leaf_pred = leaf_node.predict_one(X, tree=self)
                for key in leaf_pred.keys():
                    leaf_pred[key] = round(leaf_pred[key], 6)

                #print("standard pred : " + str(leaf_pred))
                return leaf_pred
            else:
                # print("stats")
                for key in leaf_node.stats.keys():
                    leaf_node.stats[key] = round(leaf_node.stats[key], 6)
                return leaf_node.stats
    ...
    def _get_votes_for_instanceHS(self, X, lambda_selected=0.1):
        """ Get class votes for a single instance.
        ...
        # else:
        #     return {}
        # print("inside _get_votes_for_instanceHS")

        if self._tree_root is not None:
            found_node, path = self._tree_root.filter_instance_to_leafHS(X, None, -1)
            leaf_node = found_node.node
            if leaf_node is None:
                leaf_node = found_node.parent  # until here the same with regular get_votes_for_instance
            if not isinstance(leaf_node, SplitNode):  # if it is leaf
                ...
                # Initialize sum_of_hs using Decimal from the start
                sum_of_hs = {key: 0 for key in
                             set(leaf_node.predict_one(X, tree=self)).union(self._tree_root.stats)}

                #lambda_selected = Decimal(lambda_selected)


                if len(path) == 1 or lambda_selected < 0.5:  # If the tree is just the root, return leaf prediction directly
                ...
                else:  # Otherwise, traverse the path from root to leaf
                   # print("path size more than 1, path size : " + str(len(path)))
                    while k < len(path):

                        if k == len(path) - 1:  # If we reached the leaf
                            leaf_pred = leaf_node.predict_one(X, tree=self)  #
                            parent_node = path[k - 1]
                            parent_stats = parent_node.stats
                            samples_in_parent_node = sum(parent_stats.values())

                            sum_addition = {key: 0 for key in set(leaf_node.predict_one(X, tree=self)).union(self._tree_root.stats)}

                            for key in leaf_pred.keys():
                                sum_addition[key] = leaf_pred[key] - parent_stats.get(key, 0)
                                sum_addition[key] /= 1 + (lambda_selected / samples_in_parent_node)

                                if key not in sum_of_hs:
                                    sum_of_hs[key] = 0

                                # Use Decimal to prevent underflow

                                sum_of_hs[key] = round(sum_of_hs[key] + sum_addition[key] + self._tree_root.stats.get(key, 0), 6)
                                # if round(leaf_pred[key], 6) != sum_of_hs[key] and lambda_selected == 0:
                                #     print("Error, hs is not working properly")
                                #     print("leaf pred[ : " + str(key) + "] : " + str(round(leaf_pred[key], 6)))
                                #     print("final pred[ : " + str(key) + "] : " + str(sum_of_hs[key]))


                        else:  # Intermediate nodes
                            current_node_stats = path[k].stats
                            parent_node = path[k - 1]
                            parent_stats = parent_node.stats
                            samples_in_parent_node = sum(parent_stats.values())

                            sum_addition = {key: 0 for key in set(current_node_stats).union(self._tree_root.stats)}

                            for key in current_node_stats.keys():
                                sum_addition[key] = current_node_stats[key] - parent_stats.get(key, 0)
                                sum_addition[key] /= 1 + (lambda_selected / (samples_in_parent_node))
                                ...
                                # Use Decimal to prevent underflow
                                sum_of_hs[key] = sum_of_hs[key] + sum_addition[key]


                        k += 1
                    #Final precision check with high accuracy

                    #print("hs pred : " + str(sum_of_hs))
                    return sum_of_hs
    ...
    def predictHS(self, X, lambda_selected=0.1):
        ...
        r, _ = get_dimensions(X)
        predictions = []
        y_proba = self.predict_probaHS(X, lambda_selected)
        for i in range(r):
            index = np.argmax(y_proba[i])
            predictions.append(index)
        return np.array(predictions)

    def predict_probaHS(self, X, lambda_selected=0.1):
        """ Predicts probabilities of all label of the X instance(s)
        ...
        predictions = []
        for i in range(r):
            votes = copy.deepcopy(self._get_votes_for_instanceHS(X[i], lambda_selected))
            if votes == {}:
                # Tree is empty, all classes equal, default to zero
                predictions.append([0])
            else:
