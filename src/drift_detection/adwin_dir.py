...
from src.drift_detection.base_drift_detector import BaseDriftDetector


class ADWIN_DIR(BaseDriftDetector):
    ...
    >>> # Imports
    >>> import numpy as np
    >>> from src.drift_detection.adwin import ADWIN
    >>> adwin = ADWIN()
    ...
    def __init__(self, delta=.002, window_size=100):
        super().__init__()
        # default values affected by init_bucket()
        self.delta = delta
        self.window_size = window_size # how many labels to use for LDS
        ...
        self.label_histogram = defaultdict(int) # key : label, value: count
    ...
    def add_element(self, value, label):
        ...
        label: int or float (the label of the instance)
    ...
    def delete_element(self):
        ...
        old_label = next(iter(self.label_histogram)) #get the first key (index 0 at dictionary)
        self.label_histogram[old_label] -= 1
        if self.label_histogram[old_label] <=0:
            del self.label_histogram[old_label] # remove that label from dictionary if it does not appear in instances anymore
    ...
    def gaussian_kernel(x):
        # Gaussian kernel used for KDE
        return (1 / np.sqrt(2 * np.pi)) * np.exp(-x ** 2 / 2)

    def compute_dir_weights(self):
        """Compute sample weights based on LDS of DIR
           Return : a dictionary :
                    key : label
                    value : weight
        """

        labels, counts = zip(*self.label_histogram.items()) if self.label_histogram else labels, counts = ([], [])

        if not labels:
            return {} # return an empty dict if there are no labels

        labels = np.array(labels, dtype=np.float64)
        counts = np.array(labels, dtype=np.float64)

        #KDE smoothing
        x_range = np.arange(min(labels), max(labels) + 1)  # All possible values in the range of the empirical data

        # Compute KDE for each value in x_range
        smoothed_data = []
        h = 1
        n = np.sum(counts) #total sum of counts of labels
        for x0 in x_range:  # x0 is the point
            # print(x0)
            contributions = []
            for x_i, count in zip(labels, counts):  # x_i is the every point
                # print("emp[x_i] : "+ str(emp_data[1][x_i]))
               # count = emp_data[0][x_i]
                # print("count : " + str(count))
                if count > 0:
                    distance = (x0 - x_i) / h
                    kernel_value = self.gaussian_kernel(distance)
                    contributions.append(kernel_value * count)
            smoothed_data.append(sum(contributions) / (n * h))
        label_density = np.asarray(smoothed_data, dtype=np.float64)
        sample_weights = 1 / (label_density + 1e-6)  # Avoid division by zero
        sample_weights = sample_weights / np.max(sample_weights)  # Normalize

        #Convert back to dictionary
        dir_weights = {int(label): weight for label, weight in zip(x_range, sample_weights)} #TODO : define the key of the dictionary as to handle floating point keys
