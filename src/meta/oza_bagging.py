...
from src.lazy import KNNADWINClassifier
...
class OzaBaggingClassifier(BaseSKMObject, ClassifierMixin, MetaEstimatorMixin):
    ...
    --------
    >>> # Imports
    >>> from src.meta import OzaBaggingClassifier
    >>> from src.lazy import KNNClassifier
    ...
    def predictHS(self, X, lambda_selected=0.5):
        ...
        """
        r, c = get_dimensions(X)
        proba = self.predict_probaHS(X, lambda_selected)
        predictions = []
        if proba is None:
            return None
        for i in range(r):
    ...
    def predict_probaHS(self, X, lambda_selected):
        ...
        try:
            for i in range(self.actual_n_estimators):
                partial_proba = self.ensemble[i].predict_probaHS(X, lambda_selected)
                if len(partial_proba[0]) > max(self.classes) + 1:
                    raise ValueError("The number of classes in the base learner "
                                     "is larger than in the ensemble.")
