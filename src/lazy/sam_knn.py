...
from .src import libNearestNeighbors
...
class SAMKNNClassifier(BaseSKMObject, ClassifierMixin):
    ...
    Examples
    --------
    >>> from src.lazy import SAMKNNClassifier
    >>> from src.data import FileStream
    >>> from src.evaluation import EvaluatePrequential
    ...
    def get_distances(sample, samples):
        ...
        return libNearestNeighbors.get1ToNDistances(sample, samples)
        ...
        if len(self._STMLabels) > self.n_neighbors and samplesCl.shape[0] > 0:
            ...
            for i in loopRange:
                ...
                distancesSTM = SAMKNNClassifier.get_distances(self._STMSamples[i, :],
                                                              samplesShortened)
                nnIndicesSTM = libNearestNeighbors.nArgMin(self.n_neighbors, distancesSTM)[0]
                distancesLTM = SAMKNNClassifier.get_distances(self._STMSamples[i, :], samplesCl)
                nnIndicesLTM = libNearestNeighbors.nArgMin(
    ...
    def get_maj_label(distances, labels, numNeighbours):
        ...
        nnIndices = libNearestNeighbors.nArgMin(numNeighbours, distances)
        ...
        predLabels = libNearestNeighbors.mostCommon(labels[nnIndices])
    ...
    def get_distance_weighted_label(distances, labels, numNeighbours):
        ...
        predLabels = libNearestNeighbors.getLinearWeightedLabels(labels[nnIndices], sqrtDistances)
        return predLabels
